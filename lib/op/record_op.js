...
module.exports = function(app){
	...
	// modify records
	record_op.modifyRecords = function(ssu, reqQuery, reqBody, callback){
		var ret = {
			'ok': 0,
		...
		collDao.findOne({
			...
			_ids = _.map(reqBody['_id'], function(val){
				return Number(val);
			...
			_.times(numRecords, function(i){
				...
				if (record) {
					_.each(record, function(val, field){
						if (reqBody[field].length !== numColls) {
	...
	// remove records
	record_op.removeRecords = function(ssu, reqQuery, reqBody, callback){
		var ret = {
			'ok': 0,
			'msg': 'permission denied'
		}
	...
	// add record
	record_op.addRecord = function(ssu, reqQuery, reqBody, callback){
		var ret = {
			'ok': 0,
			'msg': 'permission denied'
		}
		...
		collDao.findAndModify({
			...
			if (_.some(reqBody, function(val){
				return ! val;
			})) {
				ret['msg'] = 'info incorrect';
				callback(ret);
			...
			var record = _(reqBody).chain().pick(doc.fields).extend({
				_id: doc._next_id
			}).value();
